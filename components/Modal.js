...
const Modal = () => {
    ...
    const sendComment = async (e) => {
        ...
        await addDoc(collection(db, "postss", appContext.postId, "comments"), {
          comment: input,
    ...
    return (
        <div className='fixed to-0 left-0 z-[20] h-screen w-screen bg-[#242d34bb]' onClick={closeModal}>
            ...
            <div className='bg-[#fff] w-[350px] md:w-[650px] text-white absolute left-[50%] translate-x-[-50%] mt-[40px] p-4 rounded-[20px]'
                onClick={(e) => e.stopPropagation()}>

                <MdClose className=' text-[#000] text-[22px] cursor-pointer' onClick={closeModal} />
                ...
                <div className='relative mt-8 grid grid-cols-[48px,1fr] gap-4'>
                    ...
                    <div>
                        <div className='flex gap-2 text-[12px] md:text-[16px]'>
                            <h1 className="text-[#000]" >{post?.username}</h1>
                            <h2 className='text-gray-500'><Moment fromNow>{post?.timestamp?.toDate()}</Moment></h2>
                        </div>
                        <p className='text-[12px] text-[#000] md:text-[16px]'>{post?.text}</p>
                    ...
                    <div className='mt-4'>
                        <textarea
                            className='w-[100%] bg-transparent text-[#000] outline-none resize-none text-[18px]'
                            rows="4"
                            placeholder={`ماذا تريد تكتب؟`}
                            value={input}
                        ...
                        <div className='flex justify-between items-center'>
                            ...
                            <button
                                className="bg-[#0066ff] text-white rounded-full px-4 py-1.5 font-bold shadow-md hover:bg-[#1a8cd8] disabled:hover:bg-[#1d9bf0] disabled:opacity-50 disabled:cursor-default"
                                disabled={!input.trim()}
                                onClick={sendComment}>
                                نشر
                            </button>
